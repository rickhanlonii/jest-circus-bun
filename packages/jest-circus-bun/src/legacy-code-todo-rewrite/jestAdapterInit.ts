...
export const initialize = async ({
  ...
  setGlobalsForRuntime: (globals: any) => void;
}): Promise<{
  ...
  expect.setState({expand: globalConfig.expand});

  const runtimeGlobals: any = {
    ...globalsObject,
    expect,
  };
...
const eventHandler = async (event: Circus.Event) => {
  switch (event.name) {
    ...
    case 'test_done': {
      event.test.numPassingAsserts = expect.getState().numPassingAsserts;
